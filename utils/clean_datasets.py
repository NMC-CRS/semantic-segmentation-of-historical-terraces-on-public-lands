...
This script cleans the separated training/validation/testing datasets by removing overlapping tiles from one dataset, when applicable.
...
This script can be called by the main unet script after calling separate_datasets
...
def clean_overlapping_tiles(filenames1, filenames2, min_distance):
    '''
    Looks for overlapping tiles between datasets. When found, one of pair of tiles is assigned to one dataset, while the other is ignored.

    Parameters
    ----------
    filenames1 : list
        List of tile names in one dataset
    filenames2 : list
        List of tile names in another dataset
    min_distance : int
        Minimum distance that indicates overlap between tiles

    Returns
    -------
    filtered_filenames1 : list
        List of cleaned tile names in one dataset
    filtered_filenames2 : list
        List of cleaned tile names in the other dataset
